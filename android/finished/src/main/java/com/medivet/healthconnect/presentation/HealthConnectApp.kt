...
import kotlinx.coroutines.launch // Added to fix coroutine scope launch error
...
fun ShowAuthFlow() {
    HealthConnectTheme {
        ...
        NavHost(navController = navController, startDestination = "login") {
            composable("login") {
                LoginScreen(
                    onLoginSuccess = { }, // Auth state change will trigger recomposition
                    onNavigateToRegister = { navController.navigate("register") }
                )
            }
            composable("register") {
                RegisterScreen(
                    onRegisterSuccess = { }, // Auth state change will trigger recomposition
